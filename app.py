...
# -*- coding: utf-8 -*-
"""
CREADOR DE PROMPTS v3.0 - Refactorizado
Aplicación principal con autenticación
"""

import streamlit as st
import os
from ui.wizard import mostrar_wizard_streamlit
...
def mostrar_app_principal() -> None:
    ...
    if st.session_state.mostrar_info and st.session_state.get("paso_wizard", 1) == 1:
        with st.expander("📖 ¿Cómo funciona?", expanded=True):
            ...
            ### Características:
            - 🤖 Motor: 100% ChatGPT (gpt-4o-mini)
            - 📎 Adjunta documentos (PDF, DOCX, TXT, imágenes)
            - 👁️ Visión para imágenes y PDFs escaneados
    ...
    with col1:
        st.markdown(
            "<p style='text-align: center; font-size: 12px; color: gray;'>✨ GIRO</p>",
            unsafe_allow_html=True,
        )

    with col2:
        st.markdown(
            "<p style='text-align: center; font-size: 12px; color: gray;'>Potenciado por ChatGPT</p>",
            unsafe_allow_html=True,
        )

    with col3:
        st.markdown(
            "<p style='text-align: center; font-size: 12px; color: gray;'>v3.1</p>",
            unsafe_allow_html=True,
        )
